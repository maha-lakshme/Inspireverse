...
interface FireStoreRepository {
      suspend fun storeQuotes(userId: String?, quoteImageList: List<Pair<Quote, Photos>>):Result<Unit>
...
override suspend fun storeQuotes(userId: String?, quoteImageList: List<Pair<Quote, Photos>>):Result<Unit>{

    return suspendCancellableCoroutine { cont ->
        val userQuotesRef = fireStore.collection("users").document(userId.toString()).collection("quotes")
     ...
     override suspend fun updateLikeStatus(
         ...
         return suspendCancellableCoroutine { cont->
             ...
             val docRef= fireStore.collection("users").document(userId).collection("quotes").document(quote.id.toString())
             ...
             docRef.set(data, SetOptions.merge()).addOnCompleteListener { task ->
                 if (task.isSuccessful) {
                     ...
                     cont.resume(Result.failure(task.exception ?: Exception("Failed to update like status")))
     ...
     override suspend fun toggleLikeStatus(userId: String, quoteId: String, isLiked: Boolean): Result<Unit> {
         return suspendCancellableCoroutine { cont ->
